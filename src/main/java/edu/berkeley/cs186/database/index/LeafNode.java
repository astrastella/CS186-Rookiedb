...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        // TODO(proj2): implement
        if (keys.contains(key)) throw new BPlusTreeException("Duplicate key not allowed");
        ...
        if (keys.size() == 2 * order + 1) {
            // keep only the first d (=order) entries and move the rest to a new leaf node
            List<DataBox> newKeys = new ArrayList<>(keys.subList(order, 2*order+1));
